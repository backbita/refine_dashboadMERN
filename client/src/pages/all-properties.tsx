...
function AllProperties() {
  ...
  const { tableQueryResult: {data, isLoading, isError}, current, setCurrent, setPageSize, pageCount, 
sorter, setSorter, filters, setFilters} = useTable();

  const allProperties = data?.data ?? []

  const currentPrice = sorter.find((item) => item.field 
  === 'price')?.order;
  ...
  return (
    <Box>
      <Box mt='20px' sx={{ display: 'flex', flexWrap: 'wrap', gap: 3}}>
        ...
        fontWeight={700}
        color='#11142d'>
        {!allProperties.length ? 'There is no properties yet' : 'All Properties'}</Typography>
